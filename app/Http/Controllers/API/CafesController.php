...
class CafesController extends Controller
{
    ...
    public function postLikeCafe(Request $request,$cafeID){
        $cafe =Cafe::where('id','=',$cafeID)->first();
        $cafe->likes()->attach($request->user('api')->id,[
            'created_at'=>Carbon::now(),
            'updated_at'=>Carbon::now()
        ]);
        // return $cafe->likes()->get();
        // return response()->json(['cafe_liked'=>true],201);
    }

    public function deleteLikeCafe(Request $request,$cafeID){
        $cafe =Cafe::where('id','=',$cafeID)->first();
        $cafe->likes()->detach($request->user('api')->id);
    ...
    public function postAddTags(Request $request,$cafeID){
        ...
        Tagger::tagCafe($cafe,$tags,$userID);
    ...
    public function deleteCafeTag(Request $request,$cafeID,$tagID){
        DB::tables('cafes_users_tags')->where('cafe_id',$cafeID)->where('tag_id',$tagID)->where('user_id',$request->user('api')->id)->delete();
        return response(null,204);
    }
